...
plugin, use the `codepropertygraph-schema` and `overflowdb-traversal` packages
in your build files, together with a JSON file which defines what will be
extended. The `sample-plugin` contains a working example of the usage of the
aforementioned packages in `schema/build.sbt`, and an example of the
JSON file at `schema/src/main/resources/schema/ext.json`.

The example schema extension JSON file adds an edge of type `FOO` that is
valid from `FILE` nodes to `METHOD` nodes:
