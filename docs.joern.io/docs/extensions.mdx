...
of the schema definition at `schema/src/main/scala/schema/io/shiftleft/codepropertygraph/CpgExtSchema.scala`.

The example schema extension adds a new node base type `OTHER_INSTRUCTION`, along with an abstract base type `INSTRUCTION`
and a new property type. Then it allows to connect `FILE` nodes (from the base cpg schema) via `AST` edges with the new nodes. 

```scala
// schema/src/main/scala/schema/io/shiftleft/codepropertygraph/CpgExtSchema.scala

val instruction = builder.addNodeBaseType(name = "INSTRUCTION")
  .extendz(expression)
  .addProperties(name)

val platform = builder.addProperty(
  name = "PLATFORM", valueType = ValueTypes.STRING, cardinality = Cardinality.One)
  .protoId(2220)

val otherInstruction = builder.addNodeType(name = "OTHER_INSTRUCTION")
  .extendz(instruction)
  .addOutEdge(ast, inNode = expression)
  .protoId(2221)

val run = builder.addNodeType(name = "RUN")
  .extendz(instruction)
  .addOutEdge(ast, inNode = expression)
  .protoId(2222)

val from = builder.addNodeType(name = "FROM")
  .extendz(instruction)
  .addProperty(platform)
  .addOutEdge(ast, inNode = expression)
  .protoId(2223)

val copy = builder.addNodeType(name = "COPY")
  .extendz(instruction)
  .addProperty(platform)
  .addOutEdge(ast, inNode = expression)
  .protoId(2224)

file.addOutEdge(ast, inNode = otherInstruction)
file.addOutEdge(ast, inNode = run)
file.addOutEdge(ast, inNode = from)
file.addOutEdge(ast, inNode = copy)
```

Note that you can open `sample-plugin` in your favorite IDE (we recommend Intellij Idea) to get autocompletion etc. for creating your schema extension. To generate the domain classes and install the schema extension in your joern distribution (in `~/bin/joern`) run `./install.sh` in the sample-plugin. 